import logging
from utils import LossRecord, LpLoss
...
class BaseTrainer:
        ...
        with tqdm(total=self.epochs) as bar:
            for epoch in range(self.epochs):
                ...
                if (epoch + 1) % self.eval_freq == 0:
                    ...
                    # self.vis_error(model, test_loader.dataset[-1], epoch)
    ...
    def train(self, model, train_loader, optimizer, criterion, scheduler=None, loss_list=None, **kwargs):
        ...
        for x, y in train_loader:
            ...
            train_loss, loss_dict = self.loss(y_pred, y, criterion, loss_list=loss_list, **kwargs)
            # compute gradient
            optimizer.zero_grad()
            train_loss.backward()
            optimizer.step()
            # record loss and update progress bar
            loss_record.update({"train_loss": train_loss.sum().item()}, n=y_pred.shape[0])
            for key, loss in loss_dict.items():
                loss_record.update({key: loss.sum().item()}, n=y_pred.shape[0])
    ...
    def evaluate(self, model, eval_loader, criterion, split="valid", loss_list=None, **kwargs):
        # l1_crit = LpLoss(p=1, size_average=False)
        ...
        with torch.no_grad():
            for x, y in eval_loader:
                ...
                # compute loss
                y_pred = model(x).reshape(y.shape)
                l2_loss, loss_dict = self.loss(y_pred, y, criterion, loss_list=loss_list, **kwargs)
                loss_record.update({"{}_loss".format(split): l2_loss.sum().item(),}, n=y_pred.shape[0])
                for key, loss in loss_dict.items():
                    loss_record.update({key: loss.sum().item()}, n=y_pred.shape[0])
        return loss_record
    
    def vis_error(self, model, vis_data, epoch, **kwargs):
        model.eval()
        with torch.no_grad():
            x, y = vis_data
            x = x.unsqueeze(0)
            ...
            heatmap = plt.imshow(y_pred[0, :, :, 0], cmap='viridis', interpolation='nearest', vmin=-1.75, vmax=1.75)
            plt.colorbar(heatmap)
            plt.savefig(os.path.join(self.saving_path, "heatmap_{}.png".format(epoch)))
            plt.close()
            
            error = error.cpu().numpy()
            heatmap = plt.imshow(error[0, :, :, 0], cmap='twilight_shifted', interpolation='nearest', vmin=-0.05, vmax=0.05) # 'twilight_shifted'
            plt.colorbar(heatmap)
            plt.savefig(os.path.join(self.saving_path, "error_{}.png".format(epoch)))
            plt.close()
    
    def loss(self, y_pred, y, criterion, loss_list=None, **kwargs):
        loss_dict = {}
