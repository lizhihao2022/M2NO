...
class DiffReact2DDataset:
    def __init__(self, data_path, raw_resolution=[128, 128, 101], 
                 sample_resolution=[64, 64, 20], eval_resolution=[64, 64, 20], 
                 in_t=1, out_t=1, duration_t=10, start_x=-1, end_x=1, start_y=-1, end_y=1,
        ...
        self.train_dataset = DiffReact2DBase(train_x, train_y, mode='train')
        self.valid_dataset = DiffReact2DBase(valid_x, valid_y, mode='valid')
        self.test_dataset = DiffReact2DBase(test_x, test_y, mode='test')
    ...
    def load_data(self, data_path):
        ...
        f = File(data_path, 'r')
        for item in f.items():
            u = torch.tensor(np.array(item[1]['data']), dtype=torch.float32)
            raw_data.append(u)
    ...
    def pre_process(self, data, raw_resolution, sample_resolution, 
        ...
        if mode == 'train':
            x = data[:, :in_t, ::sample_factor_0, ::sample_factor_1]
            y = data[:, in_t:in_t+1, ::sample_factor_0, ::sample_factor_1]
            for i in range(1, duration_t):
                x = torch.concatenate((x, data[:, i:in_t+i, ::sample_factor_0, ::sample_factor_1]), axis=0)
                y = torch.concatenate((y, data[:, in_t+i:in_t+i+1, ::sample_factor_0, ::sample_factor_1]), axis=0)
        else:
            x = data[:, out_t-in_t:out_t, ::sample_factor_0, ::sample_factor_1]
            y = data[:, out_t:out_t+1, ::sample_factor_0, ::sample_factor_1]
            for i in range(1, duration_t):
                x = torch.concatenate((x, data[:, out_t+i-in_t:out_t+i, ::sample_factor_0, ::sample_factor_1]), axis=0)
                y = torch.concatenate((y, data[:, out_t+i:out_t+i+1, ::sample_factor_0, ::sample_factor_1]), axis=0)
        
        x = x.reshape(x.shape[0], x.shape[2], x.shape[3], x.shape[4])
        y = y.reshape(y.shape[0], y.shape[2], y.shape[3], y.shape[4])
        ...
        grid_y = torch.linspace(self.start_y, self.end_y, x.shape[2])
