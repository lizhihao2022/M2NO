...
class BurgersTimeDataset:
    def __init__(self, data_path, raw_resolution=[1024, 201], 
                 ...
                 **kwargs):
        data, coor_t, coor_x = self.load_data(data_path)
        if subset:
        ...
        self.train_dataset = BurgersTimeBase(train_x, train_y, mode='train')
        self.valid_dataset = BurgersTimeBase(valid_x, valid_y, mode='valid')
        self.test_dataset = BurgersTimeBase(test_x, test_y, mode='test')
    ...
    def load_data(self, data_path):
        ...
        coor_t = torch.tensor(np.array(f['t-coordinate']), dtype=torch.float32)
        coor_x = torch.tensor(np.array(f['x-coordinate']), dtype=torch.float32)
        
        return u, coor_t, coor_x
