...
class ResidualLoss(object):
    ...
    def __call__(self, r):
        return self.abs(r, torch.zeros_like(r))
...
class MaskedLpLoss(object):
    ...
    def rel(self, x, y, masked):
        if masked is not None:
            assert masked.size() == x.size(), "Masked tensor must have the same shape as input tensors."
            x = x[masked]
            y = y[masked]
    ...
    def __call__(self, x, y, masked):
        return self.rel(x, y, masked)
...
class LpLoss(object):
    ...
    loss function with rel/abs Lp loss
    '''
    def __init__(self, d=2, p=2, size_average=True, reduction=True, mask=None):
        super(LpLoss, self).__init__()
    ...
    def rel(self, x, y):        
        ...
        if self.mask is not None: 
            ...
            mask = mask.repeat(num_examples, 1, 1, 1)
            x = x[mask]
            y = y[mask]
